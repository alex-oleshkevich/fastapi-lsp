...
| `dashmap` | Concurrent maps for `WorkspaceState`; feature reads never block pass-1 writes. |
...
- **Not `async-lsp`.** Considered as the alternative to the tower-lsp lineage: it guarantees sequential notification handling and composable tower layers, but has a steeper API and fewer ergonomic helpers. We get the ordering guarantee we need from `tower-lsp-server` configuration instead (E01 REQ-ARCH-08); revisit only if that proves insufficient.
