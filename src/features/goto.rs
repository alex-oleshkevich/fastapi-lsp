...
pub fn references(
    ...
    if let Some(dep) = facts
        .dep_defs
        .iter()
        .find(|d| position_in_range(pos, d.node_id.range.start, d.node_id.range.end))
    {
        if include_declaration {
            locs.push(LspLocation {
                uri: uri.clone(),
...
mod tests {
    ...
    fn goto_client_call_navigates_to_handler() {
        use crate::state::{
            ClientCall, ClientCallSite, FileFacts, Linked, Location as StateLocation, Method,
    ...
    fn clicking_inside_fstring_interpolation_does_not_jump_to_route() {
        ...
        use crate::state::{
            ClientCall, FileFacts, Linked, Location as StateLocation, Method, ResolvedPath,
            RouteId, RouteRecord,
        };
        ...
        assert!(
            result.is_none(),
            "clicking inside f-string interpolation must not jump to route handler"
        );
    }
    ...
    fn references_on_route_handler_includes_client_call_sites() {
        use crate::state::{
            ClientCallSite, FileFacts, Linked, Location as StateLocation, Method, ResolvedPath,
            RouteId, RouteRecord,
        };
        ...
        let call_range = Range {
            start: Position::new(8, 18),
            end: Position::new(8, 26),
        };
    ...
    fn references_include_declaration_adds_handler_location() {
        ...
        assert!(
            !locs.iter().any(|l| l.range == handler_range),
            "declaration should not appear when include_declaration=false"
        );
        ...
        assert!(
            locs_with
                .iter()
                .any(|l| l.range == handler_range && l.uri == uri_app),
            "declaration should appear when include_declaration=true"
        );
    }
    ...
    fn references_on_dep_def_returns_dep_refs_and_override_sites() {
        ...
        assert!(
            ranges.contains(&override_range),
            "missing override site location"
        );
    ...
    fn references_on_dep_def_includes_type_alias_usages() {
        ...
        let alias_param_range = Range {
            start: Position::new(10, 15),
            end: Position::new(10, 37),
        };
        ...
        assert!(
            locs.iter()
                .any(|l| l.range == alias_param_range && l.uri == uri_handler),
